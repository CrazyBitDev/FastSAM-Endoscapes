...
class FastSAMPrompt:
    ...
    def __init__(self, image, results, device='cuda', clip_model=None, preprocess=None):
    ...
    def retrieve(self, model, preprocess, elements, search_text: str, device) -> int:
        ...
        tokenized_text = clip.tokenize([search_text] if isinstance(search_text, str) else search_text).to(device)
    ...
    def text_prompt(self, text):
        ...
        probs = self.retrieve(clip_model, preprocess, cropped_boxes, text, device=self.device)
        scores = probs[:, 0].softmax(dim=0)
        max_idx = scores.argsort()
        max_idx = max_idx[-1]
        max_idx += sum(np.array(filter_id) <= int(max_idx))
        return np.array([annotations[max_idx]['segmentation']]), probs
